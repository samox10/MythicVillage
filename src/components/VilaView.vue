...
const props = defineProps({
  ...
  buildingsState: { type: Array, required: true }
})

const emit = defineEmits(['spend-resources', 'finish-upgrade'])

// Dados Estáticos (Definições)
const buildingDefinitions = [
  ...
  { id: 4, key: 'centrorecrutamento', name: 'Centro de Recrutamento', type: 'RECURSO', desc: 'Recrutar novos funcionários', img: '🔮', baseCost: 5000, baseTime: 30, reqId: 1, reqLvl: 1, effectDesc: (lvl) => `+${lvl} Mythic/s` }
]

// Merge dos dados com o Save
const displayBuildings = computed(() => {
  return buildingDefinitions.map(def => {
    const state = props.buildingsState.find(b => b.key === def.key) || { level: 0 }
    const currentCost = Math.floor(def.baseCost * Math.pow(1.5, state.level))
    let status = 'idle'
    if (def.reqId) {
      const req = props.buildingsState.find(b => b.id === def.reqId)
...
// Controle de Fila
const buildingQueue = ref({}) // Armazena quem está construindo
...
// === LÓGICA DO UPGRADE (CORRIGIDA) ===
const startUpgrade = (b) => {
  // 1. Validações Locais
  if (activeBuilders.value >= maxBuilders) return
  if (props.resources.goldCoin < b.cost.gold) return

  // 2. GASTA O DINHEIRO (Imediato)
  emit('spend-resources', { gold: b.cost.gold, mythic: b.cost.mythic })

  // 3. INICIA O TIMER (Visual)
  buildingQueue.value[b.id] = { progress: 0 }
  
  const tickRate = 100 // Atualiza a cada 0.1s
  const totalTicks = (b.upgradeTime * 1000) / tickRate
  const increment = 100 / totalTicks

  const interval = setInterval(() => {
    // Verifica se ainda está na fila (segurança)
    if (buildingQueue.value[b.id]) {
      ...
      if (buildingQueue.value[b.id].progress >= 100) {
        ...
        // 5. SOBE O NÍVEL (Só agora)
        emit('finish-upgrade', b.id)
      }
    } else {
      clearInterval(interval) // Limpa se algo der errado
    }
  }, tickRate)
}
</script>
  ...
  <div class="vila-container">
    ...
    <div class="builders-hud">
      <div class="b-status">
        ...
        <div class="b-dots">
          <span 
            v-for="n in maxBuilders" 
            :key="n" 
            class="dot"
            :class="{ 'busy': n <= activeBuilders }"
          ></span>
        </div>
      </div>
    ...
    <div class="rpg-grid">
      <div 
        v-for="b in displayBuildings" 
        :key="b.id" 
        class="rpg-card"
        :class="{ 'is-locked': b.status === 'locked' }"
      >
        <div class="card-visual">
          ...
          <div v-if="buildingQueue[b.id]" class="building-overlay">
            ...
            <span class="prog-txt">{{ Math.floor(buildingQueue[b.id].progress) }}%</span>
          </div>
        </div>
        ...
        <div class="card-info">
          <div class="info-head">
            <h3>{{ b.name }}</h3>
            <span class="type-pill">{{ b.type }}</span>
          </div>

          <div class="stats-area">
            <div class="stat-line current">
              <span>Atual:</span> <strong>{{ b.currentEffect }}</strong>
            </div>
            <div class="stat-line next" v-if="b.status !== 'locked'">
              <span>Próx:</span> <strong>{{ b.nextEffect }}</strong>
            </div>
          </div>

          <button 
            class="action-btn"
            @click="startUpgrade(b)"
            :disabled="b.status === 'locked' || props.resources.goldCoin < b.cost.gold || buildingQueue[b.id] || activeBuilders >= maxBuilders"
          >
            <div v-if="b.status === 'locked'" class="btn-content locked">
              🔒 REQ: NÍVEL {{ b.reqLvl }}
            </div>

            <div v-else-if="buildingQueue[b.id]" class="btn-content construction">
              EM ANDAMENTO...
            </div>
            ...
            <div v-else class="btn-content available">
              ...
              <div class="cost-pill">
                <span class="gold-icon">G</span> {{ formatNum(b.cost.gold) }}
              </div>
            </div>
          </button>
        </div>
...
.building-overlay {
  position: absolute; inset: 0; background: rgba(15,23,42,0.9); z-index: 10;
...
/* Action Button */
.action-btn {
  margin-top: auto; border: none; border-radius: 6px; padding: 0;
  background: #38bdf8; color: #000; cursor: pointer; transition: 0.2s;
